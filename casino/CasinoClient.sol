...
contract CasinoClient is ICasinoClient {
    ...
    function getVal() public view returns (uint8 code, uint128 comment) {
        code = m_lastCode;
        comment = m_lastComment;
    }
    ...
    function betColor(uint128 value, uint8 isRed) public view onlyOwner {
        ICasino(m_casino).colorBet{value: value}(isRed);
    }

    function betGreatSmall(uint128 value, uint8 isGreat) public view onlyOwner {
        ICasino(m_casino).greatSmallBet{value: value}(isGreat);
    }

    function betParity(uint128 value, uint8 isEven) public view onlyOwner {
